...
class DatasetPreprocessing:
    ...
    def __init__(self, x_input_file, y_input_file, dataset_file):
        self.x_input_file = x_input_file
        self.y_input_file = y_input_file
        self.dataset_file = dataset_file


    def preprocess(self, x_raw, y_raw):
        ...
        self.x_feature_names = new_columns[:num_features]

        # one-hot encode y_raw
        y_transformer = OneHotEncoder()
        y_raw = np.array(y_raw["surface"]).reshape(-1,1)
        y_transformer.fit(y_raw)
        self.y_category_names = y_transformer.categories_[0]
        y_preprocessed = y_transformer.transform(y_raw).toarray()

        return (x, y_preprocessed)


    def create_dataset(self):
        x_raw = pd.read_csv(self.x_input_file)
        y_raw = pd.read_csv(self.y_input_file)
        (x_preprocessed, y_preprocessed) = self.preprocess(x_raw, y_raw)
        x_train, x_test, y_train, y_test = train_test_split(x_preprocessed, y_preprocessed, test_size=0.4)
        np.savez_compressed(dataset_file, x_train = x_train, x_test = x_test, y_train = y_train,
                            y_test = y_test, x_feature_names = self.x_feature_names, y_category_names = self.y_category_names)


if __name__ == "__main__":
    print("Preprocessing the dataset, this will take 10 - 30 seconds ...")
    x_input_file = "career-con-2019/X_train.csv"
    y_input_file = "career-con-2019/y_train.csv"
    dataset_file = "dataset_career_con_2019.npz"
    dataset = DatasetPreprocessing(x_input_file, y_input_file, dataset_file)
    dataset.create_dataset()
    print("The preprocessing is finished, the resulting numpy dataset is stored at {}.".format(dataset_file))